...
const GLint ROOF_CAP_NBR_SLICES = 10;
const GLint ROOF_CAP_NBR_STACKS = 10;

/* CENTER PIECE */
const GLfloat CENTER_PIECE_COLOR[] = { 0.8f, 0.8f, 0.2f, 1.0f };
const GLfloat CENTER_BASE_RADIUS = 0.5f;
const GLfloat CENTER_TOP_RADIUS = 0.5f;
...
const GLfloat BOTTOM_INNER_RADIUS = 1.0;
const GLfloat BOTTOM_OUTER_RADIUS = 1.0;
const GLint BOTTOM_NBR_SLICES = 10;
const GLint BOTTOM_NBR_STACKS = 10;

/* RIDE POLES */
const GLint NBR_RIDE_POLES = 4;
const GLfloat RIDE_POLE_BASE_RADIUS = 0.02f;
const GLfloat RIDE_POLE_TOP_RADIUS = 0.02f;
const GLfloat RIDE_POLE_LENGTH = 1.0f;
...
void Carousel::SetupRenderingContext()
{
  ...
  /* carousel botom stand */
  gltMakeDisk(bottom, BOTTOM_INNER_RADIUS, BOTTOM_OUTER_RADIUS, BOTTOM_NBR_SLICES, BOTTOM_NBR_STACKS);
...
void Carousel::Draw(GLMatrixStack &modelViewMatrix, GLShaderManager &shaderManager, GLGeometryTransform &transformPipeline, M3DVector4f &vLightEyePos)
{
  ...
  for ( int i = 0; i < NBR_RIDE_POLES; i++ )
  {
    ...
    modelViewMatrix.PushMatrix();
      modelViewMatrix.Translate(cos(D2R(rot)), sin(D2R(rot)), 0.0f);
      shaderManager.UseStockShader(GLT_SHADER_POINT_LIGHT_DIFF, transformPipeline.GetModelViewMatrix(), 
